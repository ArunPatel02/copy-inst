...
function MyTabBar({ state, descriptors, navigation, ...rest }) {
  ...
  return (
    <StyledView className="h-20 w-full items-center justify-center absolute bottom-3 left-0 right-0 shadow-2xl">
      <StyledView
        className="h-full rounded-full bg-white w-11/12 flex flex-row items-center justify-between p-1"
        style={styles.boxShadow}
      >
        <Animated.View
          className="w-1/3 h-full rounded-full bg-[#393ec5] flex flex-row items-center justify-center absolute left-1"
              ...
              {index === 0 ? (
                <TabStyleView key={index}>
                  <Feather
                    name="menu"
                    size={32}
                    color={isFocusedSelected ? "white" : "black"}
                  />
                </TabStyleView>
              ) : index === 1 ? (
                <TabStyleView key={index}>
...
const Home = () => {
  return (
    <>
      <Tab.Navigator
        tabBar={(props) => <MyTabBar {...props} style={styles.boxShadow} />}
      >
        <Tab.Screen
          name="Posts"
          component={Post}
          options={({ navigation, route }) => ({
            headerTitle: (props) => (
              <StyledText className="text-xl font-semibold">Posts</StyledText>
            ),
            headerRight: () => (
              <FontAwesome5
                name="edit"
